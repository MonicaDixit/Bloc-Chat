  ...
  if(roomname){
  	...
  	io.socket.post('/room' , {name:roomname} , function(data){
  		...
  		createPublicRoom({id: data.id , name:data.name});
  		 io.socket.post('/room/'+data.id+'/users', {id: window.me.socketId});
  	});
  }
...
function addRoom(room){
	...
	var option = $('<option id="'+"room-"+room.id+'" data-name="'+room.name+'" value="'+room.id+'">'+room.name+' </option>');
...
function onClickSendPublicMessage(e) {
  ...
  addMessageToChatRoom(window.me.username, roomId, message);

  // Send the message
  io.socket.post('/chat/public', {room: roomId, msg: message, user: window.me.username});

}

// Add HTML for a new message in a public room
function addMessageToChatRoom(senderName, roomId, message) {
...
function receiveRoomMessage(data) {
  ...
  addMessageToChatRoom(senderId, room.id, data.msg);
